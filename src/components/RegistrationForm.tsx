        ...
        {submittedData ? (
          ...
          <>
            <div className="text-center mb-24">
              <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} className="inline-block p-4 bg-primary/10 rounded-full mb-8 border border-primary/20">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <Input {...register("teamName")} className="bg-black/60 border-primary/30 h-14 text-xl tracking-wider pl-4 rounded-none" placeholder="e.g. Wukong's Shadow" />
                            ...
                            <TooltipTrigger asChild>
                              <div className={`flex items-center space-x-3 p-4 border transition-all duration-300 ${isDisabled ? "opacity-40 bg-black/20 border-white/5 cursor-not-allowed" : "bg-black/40 border-primary/10 hover:border-primary/40"}`}>
                                ...
                                <div className="flex flex-col flex-grow cursor-pointer" onClick={() => !isDisabled && handleEventToggle(event.id)}>
                                  <label htmlFor={event.id} className={`text-sm font-body cursor-pointer select-none ${isDisabled ? 'text-muted-foreground' : 'text-foreground'}`}>{event.name}</label>
                                  <span className="text-[10px] text-primary/40 flex items-center gap-1"><Clock size={10} />{event.start} – {event.end}</span>
                                </div>
                              </div>
                            </TooltipTrigger>
                            {isDisabled && !isChecked && <TooltipContent className="bg-secondary text-white border-none font-headline text-[10px] tracking-widest"><p>{isDisabled}</p></TooltipContent>}
                          </Tooltip>
                        );
                      })}
                    </div>
                      ...
                      <Card className="stone-tablet border-accent/40 shadow-gold-glow overflow-hidden group">
                        <CardHeader className="border-b border-accent/20 bg-primary/5 p-6 flex flex-row items-center justify-between">
                          <CardTitle className="text-xl md:text-2xl text-primary font-headline tracking-widest flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full border border-primary/30 flex items-center justify-center text-sm font-headline bg-black/40">{index + 1}</div>
                            Disciple {index + 1}
                          </CardTitle>
                        </CardHeader>
              ...
              <div className="text-center pt-24">
                <Button type="submit" disabled={isSubmitting} className="bg-primary hover:bg-primary/90 text-black font-black px-24 py-12 text-3xl ornate-border shadow-[0_0_60px_rgba(200,155,60,0.4)] rounded-none transition-all w-full md:w-auto">
